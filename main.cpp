...
void showSettingsGui(const Settings &settings) {
    GtkEntry *entries[5];
    ...
    std::string button1Text = std::to_string(settings.sButton);
    ...
    std::string button2Text = std::to_string(settings.sButton2);
...
int main(const int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::string arg = argv[i]; arg == "--debug") {
            ...
            showSettingsGui(handler.getSettings());
            return 0;
        }
    }
